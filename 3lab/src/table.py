...
class Table:
    ...
    def __create_table(self, data: list, formula: str) -> list:
        ...
        for i in range(len(VALUES)):
            table[i].result = self.result(formula, table[i].x1, table[i].x2, table[i].x3)
        return table

    @staticmethod
    def result(formula: str, x1: int, x2: int, x3: int) -> int:
        stack = []
        polska = Table.__translation_to_polska(formula, x1, x2, x3)
        for i in polska:
            if i.isdigit():
                stack.append(i)
            elif i == '+':
